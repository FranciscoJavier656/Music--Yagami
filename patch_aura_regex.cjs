...
code = code.replace(/\{dominantColor && \(\s*<div\s*className="absolute inset-0 opacity-20 dark:opacity-30 mix-blend-screen dark:mix-blend-lighten pointer-events-none transition-colors duration-1000"\s*style=\{\{\s*background: `radial-gradient\(circle at 50% 0%, \$\{dominantColor\} 0%, transparent 70%\)`\s*\}\}\s*\/>\s*\)\}/, newAura.trim());
