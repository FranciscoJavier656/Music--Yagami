...
import { Music, Play, Disc, Trash2, Heart, ListMusic, User, Search, Filter, ChevronLeft, DownloadCloud, MoreVertical } from 'lucide-react';
...
export default function LibraryTab() {
  ...
  const items = useMemo(() => {
    ...
    return rawData.map((item: any) => {
      ...
      let subtitle = '';
      let type = activeTab; // albums, artists, tracks, playlists
